...
//    [self addSubview:saveButton];
    if (self.imageCount > 1) {
        _pageController = [[UIPageControl alloc] initWithFrame:CGRectMake(0, [[UIScreen mainScreen] bounds].size.height - 40, [[UIScreen mainScreen] bounds].size.width, 10)];
        _pageController.numberOfPages = self.imageCount;
        _pageController.currentPage = self.currentImageIndex;
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    if (self.imageBlock != nil) {
        self.imageBlock(_pageController.currentPage);
    }
...
- (void)showFirstImage
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        ...
        sourceView = self.sourceImagesContainerView;
//        .subviews[self.currentImageIndex];
    }
