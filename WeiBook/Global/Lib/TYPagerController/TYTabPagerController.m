...
- (void)configireTabPropertys
{
    ...
    _numberTextFont = [UIFont systemFontOfSize:8.0];
...
- (void)setDelegate:(id<TYTabPagerControllerDelegate>)delegate
{
    ...
    _tabDelegateFlags.configreNumberReusableCell = [self.delegate respondsToSelector:@selector(pagerController:configreCell:forItemNumber:atIndexPath:)];
...
- (void)setDataSource:(id<TYPagerControllerDataSource>)dataSource
{
    ...
    _tabDataSourceFlags.numberForIndex = [self.dataSource respondsToSelector:@selector(pagerController:numberForIndex:)];
...
- (void)layoutTabPagerView
{
    ...
    _pagerBarView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, self.contentTopEdging+[self statusBarHeight]);
    _collectionViewBar.frame = CGRectMake(0, [self statusBarHeight], [[UIScreen mainScreen] bounds].size.width, self.contentTopEdging);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_tabDataSourceFlags.numberForIndex) {
        NSString *number = [self.dataSource pagerController:self numberForIndex:indexPath.item];
        if (_tabDelegateFlags.configreReusableCell) {
            [self.delegate pagerController:self configreCell:cell forItemNumber:number atIndexPath:indexPath];
        }
        if (_tabDelegateFlags.transitionFromeCellAnimated) {
            [self.delegate pagerController:self transitionFromeCell:(indexPath.item == self.curIndex ? nil : cell) toCell:(indexPath.item == self.curIndex ? cell : nil) animated:NO];
        }
